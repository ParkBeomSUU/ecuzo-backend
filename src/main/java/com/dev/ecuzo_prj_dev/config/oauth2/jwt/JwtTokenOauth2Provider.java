...
@Slf4j
@Component
public class JwtTokenOauth2Provider {
    private String SECRET_KEY="VlwEyVBsYt9V7zq57TejMnVUyzblYcfPQye08f7MGVA9XkHa";
    private Key key;

    public JwtTokenOauth2Provider() {
        byte[] keyBytes =Base64.getDecoder().decode(SECRET_KEY);
        this.key = Keys.hmacShaKeyFor(keyBytes);
    }
    private final long TOKEN_VALID_TIME = 1000*60; //1시간
    /*1000*60*60 1시간
    1000*60*30 30분
    *
    * */
//
    ...
    public Oauth2TokenInfo createToken(String email, String role){
        log.info("Oauth2 토큰 생성 실행");
        log.info(role);
        Date now = new Date();
        // 정보 저장
    ...
    public Authentication getAuthentication(String accessToken) {
        ...
        log.info("Token Inside email :"+claims.getSubject());
